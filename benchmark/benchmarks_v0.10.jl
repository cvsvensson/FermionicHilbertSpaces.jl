...
SUITE["generate_states"]["int"] = @benchmarkable FermionicHilbertSpaces.generate_states($H, $constraint; leaf_processor=FermionicHilbertSpaces.CombineFockNumbersProcessor{FockNumber{Int}}())
...
H = hilbert_space(f, 1:N)
constraint = prod(FermionicHilbertSpaces.weighted_number_branch_constraint(allowed, w, H) for (allowed, w) in zip(allowed_ones, weights))
SUITE["generate_states"]["big_int"] = @benchmarkable FermionicHilbertSpaces.generate_states($H, $constraint; leaf_processor=FermionicHilbertSpaces.CombineFockNumbersProcessor{FockNumber{BigInt}}())
