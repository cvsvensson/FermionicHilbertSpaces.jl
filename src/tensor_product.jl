...
function tensor_product(spaces, constraint::AbstractConstraint)
    ...
    # generate constrained states #TODO: Only some constraints can use the generate_states alg. We should split the constraints up into those that can be passed to generate states and those that can't. The latter can be handled by filtering the states generated by the former.
...
sector_function(constraint::AbstractConstraint, full_space::AbstractHilbertSpace, spaces) = missing
