...
internal_rep(state::FockNumber, ::AbstractHilbertSpace, ::Type{T}=UInt64) where T = InternalRep{T}(T(state.f))
physical_rep(state::InternalRep{T}, ::Type{FockNumber{I}}) where {T,I} = FockNumber{I}(state.data)
